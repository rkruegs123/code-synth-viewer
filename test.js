...
var NUM_STIMULI = 5
var stimuli_times = Array(10000,30000,60000);
...
var keystrokes_file = "./keystrokes/keystrokes-" + participant_id + ".txt"
var answers_file = "./answers/answers-" + participant_id + ".txt"
var csv_header = "participant-id,category,timestamp,prompt,answer,stimulus-id,previous-delay\n"
...
var stim_index = -1;
var sequence = [];
var category_finished = [false, false, false, false]
...
app.get('/nextstim', function ( request, response ) {
    ...
    if (stim_index > (NUM_STIMULI - 1)) {
      stim_index = -1
      category_finished[stim_category] = true;
      stim_category = (stim_category + 1) % 4;
      should_stop = 1;
    }

    if (category_finished[stim_category]) {
      should_stop = 1
      stim_category = (stim_category + 1) % 4;
    }
...
app.get('/write_answer', function ( request, response ) {
    ...
    var csv_line = participant_id + ',' + stim_category + ',' + now('milli') + ',' + prompt + ',' + area + ',' + index + ',' + prev_delay +'\n'
